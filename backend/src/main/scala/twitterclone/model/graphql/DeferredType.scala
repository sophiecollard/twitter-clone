...
object DeferredType {
  ...
  final case class TweetsByUserId(userId: Id[User], pagination: TweetPagination)
  ...
  final case class CommentsByUserId(userId: Id[User], pagination: CommentPagination)
    extends Deferred[List[Comment]] with DeferredType
